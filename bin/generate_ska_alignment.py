...
if __name__ == "__main__":
    ...
    if os.path.exists(args.out + '.skf'):
        subprocess.check_output('ska weed --min-freq 1 --filter no-ambig --filter-ambig-as-missing --no-gap-only-sites ' + \
                                ' -o ' + args.out + '_filtered.skf ' + args.out + '.skf',
                                shell = True)
    else: 
        sys.stderr.write('ska build failed\n')
        sys.exit(1)

    # Run ska mapping
    if os.path.exists(args.out + '_filtered.skf'):
        # tmp_aln = os.path.join(os.path.dirname(args.out), 'tmp.' + os.path.basename(args.out))
        subprocess.check_output('ska map -o ' + args.out + '.fasta' + ' --threads ' + str(args.threads) + \
                                ' --ambig-mask --repeat-mask ' +  \
                                args.reference + ' ' + args.out + '_filtered.skf',
                                shell = True)
    else:
        sys.stderr.write('ska filtering failed\n')
        sys.exit(1)
    ...
    if not args.no_cleanup:
        subprocess.check_output('rm ' + args.out + '.skf ' + args.out + '_filtered.skf',
                                shell = True)
